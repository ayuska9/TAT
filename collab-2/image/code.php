...
if(isset($_POST['update_package'])){
    ...
    if($query_run){
        if($_FILES['new_image']['name']!=''){
            // If a new image has been uploaded, move it to the "package" directory with the new filename.
            move_uploaded_file($_FILES["new_image"]["tmp_name"],"package/".$update_filename);
    
            // If the package had an old image, delete it from the "package" directory.
            if($image!='' && $image != $update_filename){
                unlink("package/".$image);
            }
            // If no new image was uploaded, set a session variable to indicate that the package was updated and redirect to the index page. 
            $_SESSION['status']=" Package updated successfully.";
            header('location: index.php');
            exit;
    
        } else {
            // Set a session variable to indicate that the package was not updated and redirect to the index page.
